...
| how many gates does a tick run? | `harvest.land_combined` | one (B-0040): every eligible branch rebased in turn onto one head off `origin/<trunk>`, gated once, pushed once — each lands at that sha; a red gate bisects the set until a branch is red alone, and that one is held with its line (`conventions.harvest.gate: per-branch` keeps one gate per landing; `branches_per_tick` is the safety valve, B-0031); each gate command runs in its own process group and is killed whole past `harvest.gate_timeout_s` (600 s), the branch held with `gate timed out after N s` (B-0072) |
