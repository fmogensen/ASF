...
def latest_release():
    ...
    root = _checkout_root()
    if root:
        out = _checkout_git(root, 'for-each-ref', '--format=%(refname:short) %(creatordate:unix)',
                            'refs/tags/v*') or ''
        found = newest([tuple(line.split(' ', 1)) for line in out.splitlines() if ' ' in line])
        if found:
            stamp_ = found[1].strip()
            when = datetime.datetime.fromtimestamp(int(stamp_), datetime.timezone.utc) \
                if stamp_.isdigit() else None
            return found[0], when
