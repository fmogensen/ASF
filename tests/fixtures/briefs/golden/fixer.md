...
Why this session exists: REVIEW → FIX — changes requested, r1
...
- Every commit subject names the item: `fix(T-0001): <what>`. Harvest holds a branch whose commits do not name it; the id inside the branch name does not count.
...
Stay inside `writes:` — app/checkout/attempts.py, tests/test_checkout.py
